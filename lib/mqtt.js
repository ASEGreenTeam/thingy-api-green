...
//var client  = MQTT.connect('mqtt://mqtt.thing.zone', { 'port': 1895, 'username': 'green', 'password': '1ea713ca2e' })
...
// Variable that tells if door is fully closed or not
...
async function handleMessage(topic, message, packet) {
  ...
  //console.log('Topic: ' + thingy_service);
 ...
 //check if door fully closed through color sensor
  if(thingy_service[2].replace(/-/g, '') == TES_COLOR_UUID) {

   var color = {
        red :   message.readUInt16LE(0),
        green : message.readUInt16LE(2),
        blue :  message.readUInt16LE(4),
        clear : message.readUInt16LE(6)
}
	 ...
	 //to detect if door is open messurements of laser pointer are needed, extra brackets for visability
	 /*if((0 <= r_8.toFixed(0) && r_8.toFixed(0) <= 255) && (0 <= g_8.toFixed(0) && g_8.toFixed(0) <= 255)  && (0 <= b_8.toFixed(0) && b_8.toFixed(0) <= 255) ) {
		 door_closed = true;
		 reset_afer_close();
	 }
...
function detect_event(){
  ...
  general_avg = general_avg + avg; 
  console.log('Average' + general_avg)
}
...
/* function reset_after_close() {
	all things that have to be reseted after closing door
}*/
