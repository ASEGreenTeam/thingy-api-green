...
async function handleDoorMovement(uuid, movements) {
  ...
  if (user.alarm) {
    ...
    if (unixTimestamp > user.alarmActivationTime + user.alarmDelay) {
      ...
      if(user.telegramChatId!=0 && user.telegramAlert){
        sendTelegramAlarm(user)
      }
    }
  }
}
...
async function sendTelegramAlarm(user){
  let user = await Models.User.findOne({ username: user.username }).exec();
  console.log(user.usename);
  const bot = new TeleBot('743339121:AAF7oNxcNY0zLS2HHNmVSpQV9OU7CK9NBS8');
  bot.connect();
  bot.sendMessage(user.telegramChatId, "Alarm! Alarm!!");
  bot.stop();
}
