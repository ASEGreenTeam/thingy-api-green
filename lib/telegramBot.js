...
use command /logout to unlink yuur account from this chat
...
async function logout(bot,msg) {
  let user = await Models.User.findOne({ telegramChatId: msg.from.id}).exec();
  if(user){
    await Object.assign(user, {telegramChatId: 0, telegramAlert: false})
    user.save();
    return bot.sendMessage(msg.from.id, "Logout success!! Your chat is no more linked to any account", { replyToMessage: msg.message_id});
  }
  else {
...
async function snapshot(bot,msg) {
  ...
  if(user){
    ...
    return bot.sendMessage(msg.from.id, "Snapshot taken you can find the photos under /photos on your graphical interface", { replyToMessage: msg.message_id });
  }
  else {
    const text = "We didn't found any user with this chatId";
    return bot.sendMessage(msg.from.id, text, { replyToMessage: msg.message_id });
